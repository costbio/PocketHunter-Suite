...
- **Do not claim optimal clustering parameters.** In the deployed `PocketHunter/pockethunter.py`, DBSCAN fits with `metric='hamming'` (line 333) while the silhouette score that selects `eps` and `min_samples` is computed with the default euclidean metric (line 339). Corrected during Task 5 against an earlier, wider claim in this plan: the *data* does match — `df.pop('Frame')` at line 320 removes the only non-feature column before the loop, so `df` and `df[cols_2cluster]` are the same columns. Only the metric disagrees. Task 5 additionally verified that the `-1` noise labels are passed to `silhouette_score` unfiltered, so noise is scored as if it were a cluster.
