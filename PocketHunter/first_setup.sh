...
# Download and extract P2Rank into the repo folder. Retry on transient
# network failures — this runs inside the Docker image build, and a
# corp-firewall blip or GitHub CDN hiccup would otherwise fail the
# entire build with no recovery.
echo "Downloading P2Rank..."
wget --tries=5 --waitretry=5 --retry-connrefused \
    -q -O "$INSTALL_DIR/p2rank_2.5.tar.gz" \
